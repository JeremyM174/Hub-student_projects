# :book: Tinder: the docs

### :bookmark: Table of contents

1. [The tools](#hammer_and_wrench-the-tools)
2. [The files](#books-the-files)
3. [The reasoning](#thought_balloon-the-reasoning)

---

### :hammer_and_wrench: The tools

* Environment manager: Anaconda
* Environment: see the `.yaml` file
* IDE: VScode

### :books: The files

You may find in this repository:

* The `.ipynb` notebook which stores the entirety of the work,
* The `.csv` source dataset,
* The `.doc` file detailing the structure and contents of the dataset,
* The `.yaml` file storing the environment's specs to run the notebook.

### :thought_balloon: The reasoning

Since the work here is all about visualisations and descriptive statistics, it wouldn't make sense to leave text apart.

The `.ipynb` notebook is meant to be read like a book and follows a logic, offering insights progressively. Please refer to its contents!